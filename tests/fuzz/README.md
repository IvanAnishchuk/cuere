...
- `atheris/harness.py` — shared bootstrap; re-exports `atheris`.
- `atheris/fuzz_*.py` — one fuzz target each. Plain scripts with a top-level
  `atheris.Setup(...)` / `atheris.Fuzz()`; run directly, never imported by the
  test suite (they would fuzz forever on import).
- `test_atheris_targets.py` — a pytest smoke test that runs each target for a
  few seconds and fails on a crash. Gated behind the `atheris` marker, so the
  default `uv run pytest` skips it.
