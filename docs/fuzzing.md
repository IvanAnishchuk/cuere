...
target found [#99](https://github.com/IvanAnishchuk/cuere/issues/99): a
huge-exponent `amount`/`value` (e.g. `"1E999999999"`) passes the finiteness and
sign checks, then raises an unhandled `MemoryError` — a denial of service —
instead of the documented `WalletURIError`, because the magnitude-dependent
`format()` / `int()` runs *before* the value is bounded. That is precisely the
