"""Regenerate the terminal-QR art embedded in the cookbook docs.

Each ``<!-- qr: KEY -->`` marker in a doc is followed by a fenced ``text`` block
...
encoder and renderer. The quiet-zone trailing spaces are significant, so the
affected cookbook pages are excluded from the trailing-whitespace pre-commit
hook.

Usage:
    uv run python scripts/render_cookbook_qr.py           # rewrite docs in place
    uv run python scripts/render_cookbook_qr.py --check    # fail if stale (CI)
"""
...
# KEY -> the URI whose rendered QR is embedded under ``<!-- qr: KEY -->``.
...
# Tolerate trailing whitespace: this page is excluded from the trailing-whitespace
# pre-commit hook, so a stray space after the marker must not silently skip it.
_MARKER = re.compile(r"^<!-- qr: (?P<key>[A-Z0-9_]+) -->\s*$")
_FENCE = "```"


class CookbookError(Exception):
    """A cookbook page is malformed (a ``qr:`` marker without its fenced block)."""
...
def regenerate(text: str) -> str:
    ...
    while index < len(lines):
        ...
        if index >= len(lines) or not lines[index].startswith(_FENCE):
            ...
            raise CookbookError(msg)
        out.append(lines[index])  # opening fence (e.g. ```text)
        index += 1
...
def _report(*, check: bool, stale: list[str]) -> None:
    if not stale:
        console.print("[green]cookbook QR art is up to date[/]")
        return
    names = ", ".join(stale)
    if check:
        console.print(f"[red]stale[/] cookbook QR art — run scripts/render_cookbook_qr.py: {names}")
        raise typer.Exit(1)
...
def main(
    *,
    ...
    """Rewrite (or, with --check, verify) the QR art embedded in docs/cookbook/."""
    stale: list[str] = []
